...
const Header = () => {
  ...
  useEffect(() => {
    // Adjust header position after mount
    setTimeout(() => {
        ...
        <div className="flex h-16 items-center justify-between">
          <div className="flex-1 md:flex md:items-center md:gap-12">
            <a className="block h-full text-teal-600" href="#">
              ...
              <img
                style={{
                  filter: "brightness(0) saturate(100%) invert(100%) sepia(100%) saturate(0%) hue-rotate(129deg) brightness(101%) contrast(103%)",
                }}
                className="h-12"
                src={LogoWithOutText}
                alt="logo"
          ...
          <div className="md:flex md:items-center md:gap-12">
            <nav aria-label="Global" className="hidden md:block">
              <ul className="flex items-center gap-6 text-sm font-bold">
                {tab.map((item, index) => (
                  <li key={index}>
                    <a className="text-white transition hover:text-[#44B78B]" href="#">
                      {item}
                    </a>
                  </li>
                ))}
            ...
            <div className="flex items-center">
              <div className="block md:hidden">
                ...
                <button onClick={toggleMenu} className="rounded p-2 text-gray-600 transition hover:text-gray-600/75">
              ...
              <div className="md:hidden block">
                <div onClick={toggleMenu} className={`${isMenuOpen ? "block" : "hidden"} backdrop-blur-sm bg-[#000000]/50 w-full h-screen absolute top-0 left-0`} />

                <nav className={`transform transition-all duration-300 absolute bg-[#0C4B33] z-20 right-0 top-0 h-screen w-1/2 ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
