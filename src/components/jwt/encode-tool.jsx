...
const DecoderTool = () => {
  ...
  const defaultSecretKeys = {
    HS256: { key: keys.HS256 },
    HS384: { key: keys.HS384 },
    HS512: { key: keys.HS512 },
    ...
    PS256: { key: keys.PS256 },
    PS384: { key: keys.PS384 },
    PS512: { key: keys.PS512 },
  };
  ...
  const EncodeJWT = async () => {
    try {
      if (
        ...
        setAlertInfo({ type: "error", isShow: true, message: "Secret key must be a symmetric key" });
        return;
      }
      ...
      try {
        ...
        setAlertInfo({ type: "error", isShow: true, message: "Invalid JSON payload" });
        return;
      }
      ...
      setAlertInfo({ message: "Something went wrong, please try again!", type: "error", isShow: true });
    }
  };
        ...
        <div className="flex flex-col md:flex-row gap-8">
                    ...
                    {isOpen && (
                      <div className="select-none options-container absolute mt-1 bg-white border border-gray-400 w-full rounded-b-lg shadow-lg">
                        {algorithmOptions.map((option) => (
                          <div
                            key={option.value}
                            className={`option hover:bg-gray-200 relative py-[6px] px-[8px] text-[13px] font-medium cursor-pointer ${UserAlgorithm === option.value ? "bg-gray-200" : ""}`}
          ...
          <div className="flex flex-col justify-start md:w-[75%] relative">
            <p className="py-[6px] px-[8px] uppercase w-full text-[12px] md:text-[13px] font-thin w-full border-t border-r border-l border-gray-400">
              {["HS256", "HS384", "HS512"].includes(UserAlgorithm) ? "Secret Key" : "Private Key"}
            </p>
            <textarea
              spellCheck="false"
              value={["HS256", "HS384", "HS512"].includes(UserAlgorithm) ? SecretKey : PrivateKey}
              onChange={(e) => ["HS256", "HS384", "HS512"].includes(UserAlgorithm) ? setSecretKey(e.target.value) : setPrivateKey(e.target.value)}
              className="outline-none rounded-b-lg border border-gray-400 p-2 min-h-[120px]"
              style={{ whiteSpace: "pre-wrap", fontFamily: "monospace" }}
              type="text"
            />
            <button
              onClick={() => handlePaste(["HS256", "HS384", "HS512"].includes(UserAlgorithm) ? setSecretKey : setPrivateKey, setIsSecretKeyPasted)}
              className="absolute bottom-1 right-1 bg-white rounded-lg p-2"
            >
              {!isSecretKeyPasted ? <PasteIcon /> : <PastedIcon />}
            </button>
            <button
              onClick={() => ["HS256", "HS384", "HS512"].includes(UserAlgorithm) ? setSecretKey(defaultSecretKeys[UserAlgorithm].key) : setPrivateKey(defaultSecretKeys[UserAlgorithm].key)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="flex flex-col">
            <div className="flex flex-col relative">
              <p className={`${TokenInputFocused ? "border-blue-600" : "border-gray-400"} py-[6px] px-[8px] uppercase text-[12px] md:text-[13px] font-thin w-full border-t border-r border-l`}>
                PayLoad
              </p>
              <textarea
          ...
          <RippleButton
            onClick={EncodeJWT}
            rippleClass="bg-[#51be95]"
            className="h-full md:hidden block font-bold text-sm md:text-[15px] rounded-full bg-[#44B78B] py-3 md:p-0"
          >
            <span>Encode</span>
          </RippleButton>
            ...
            <RippleButton
              onClick={async () => {
                if (!userJWTData.token) {
                  setAlertInfo({ message: "Nothing to copy", type: "info", isShow: true });
                  return;
                }

                try {
                  await navigator.clipboard.writeText(userJWTData.token);
                  setAlertInfo({ message: "Copied", type: "success", isShow: true });
                } catch (error) {
                  console.error("Failed to copy JWT:", error);
                  setAlertInfo({ message: "Failed to copy", type: "error", isShow: true });
                }
              }}
              rippleClass="bg-[#51be95]"
